...
Thanks for looking. Ordo is a local-first desktop app: Electron UI + a Python sidecar. Please keep user files on disk untouched.

谢谢。Ordo 是本地优先的桌面应用（Electron + Python sidecar）。**默认不要移动、复制或改名用户的原文件。**
...
3. If you touch `apps/desktop/**`, run `npm test` there. Inline script changes in `renderer/index.html` need `node scripts/csp-hash.js --write`.
4. Do not commit `node_modules/`, `.venv/`, `dist/`, or live `*.db` files.
5. Do not add telemetry, extra network calls from the renderer, or cloud embedding without an explicit design discussion.

## Issues

Bugs and ideas: GitHub Issues. Security: see [`SECURITY.md`](SECURITY.md), not a public issue.

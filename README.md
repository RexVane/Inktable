...
| 带引用问答（四条后置校验留痕，可审计） | ✅ |
| 问答快速/深度双档（快速=简洁无引用单轮；深度=引用+逐条校验） | ✅ |
| 意图路由（通用问题直接答，界面标注来源） | ✅ |
| 多轮对话浓缩 + 拒答自动重试 + 句级自动归因 | ✅ |
| 推理模型自适应（探测/辅助调用空正文自动放宽预算；中转站 UA 兼容） | ✅ |
| 模型配置三分区（问答/知识馆整理/向量；接口格式可选，本地 Ollama 自动识别模型列表） | ✅ |
| 知识馆整理（AI 可新建分类/标签；云端并发批量清空，可中途停止） | ✅ |
...
- **知识问答**：接口格式可选 —— OpenAI 兼容接口（云端/中转，chat/completions）
  或本地 Ollama（免密钥，地址自动补 `/v1`，自动识别已装模型列表）；
  也可从本机 cc-switch 一键导入供应商
...
「AI 整理全部」循环跑批（每批 20 篇）直到全部整理完，进行中可再点
停止。整理模型优先复用现有分类/标签词表，不合适时**自动创建新分类
与新标签**（同名自动折算，不重复建）；云端接口 4 路并发生成。
「重建相关资料」复用既有 bge-m3 向量构建条目关系，不上传不复制文件。

## 已知限制

- 无应用图标，未代码签名（首次打开需右键→打开）
- 原文查看器仅 PDF / DOCX 有原版式渲染，其余格式为提取文本视图；
  超大 PDF（数百 MB）整载进渲染层，首次打开较慢

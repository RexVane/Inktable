...
「AI 整理全部」会创建一个有持久化 `run_id` 的有界任务，每批最多 20 篇；
同一轮每个条目最多尝试一次，失败项只在用户点击「重试失败项」后重新进入。
进行中再次点击会停止领取下一批，并明确等待当前批次安全完成。整理模型优先
复用现有分类/标签词表，不合适时**自动创建新分类与新标签**；名称经过
Unicode、大小写和空白归一化，同名自动折算。云端接口最多 4 路并发生成。

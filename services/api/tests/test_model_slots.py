...
def test_library_env_fallback(monkeypatch):
    monkeypatch.setenv("ORDO_LIBRARY_MODEL", "env-model-7b")
...
def test_discover_prefers_explicit_env(monkeypatch):
    monkeypatch.setenv("ORDO_OLLAMA_URL", "http://127.0.0.1:6553")
...
def test_discover_picks_second_candidate_when_first_is_dead(
    ...
    monkeypatch.delenv("ORDO_OLLAMA_URL", raising=False)
